...
echo "########## [2] 권한 관리 ##########" > $RESULT_FILE
echo "" >> $RESULT_FILE # \r\n
echo "===== [2.1 - 2.3] 서비스 정책 관리 =====" >> $RESULT_FILE
for GROUP_POLICY in `cat $CURRENT_PATH/tmp_group.txt`
do
    echo "> Group Name : $GROUP_POLICY" >> $RESULT_FILE
    GRP_POLICY=$(aws iam list-attached-group-policies --group-name $GROUP_POLICY --output text | awk '{print $3}')
...
echo "" >> $RESULT_FILE #\r\n
echo "<[2] 권한 관리> 점검 완료." >> $RESULT_FILE

echo "########## [3] 가상 리소스 관리 ##########" >> $RESULT_FILE
echo "" >> $RESULT_FILE #\r\n
echo "===== [3.1] 보안그룹 인/아웃바운드 ANY 설정 관리 =====" >> $RESULT_FILE

SGID=$(aws ec2 describe-security-groups --query "SecurityGroups[].GroupId" --output text)

echo "Checking 'All Protocol' in Security Group (Except HTTP & HTTPS)..." >> $RESULT_FILE
echo "=========================== EGRESS ===========================" >> $RESULT_FILE
for PortChk in $SGID
do
    if [ ${PortChk} ]; then
		EgressPortAll=$(aws ec2 describe-security-groups --filter Name=vpc-id,Values=$VPC --query "SecurityGroups[?GroupId=='${PortChk}'&&IpPermissionsEgress[?IpProtocol=='-1']].{Egress: IpPermissionsEgress[?IpProtocol=='-1'].{IpProtocol: IpProtocol}}" --output text)
		if [ "${EgressPortAll}" ] ; then
			echo "${PortChk}" >> $RESULT_FILE
		fi
	fi
done
echo "==============================================================" >> $RESULT_FILE
echo "" >> $RESULT_FILE
echo "========================== INGRESS ==========================" >> $RESULT_FILE
for PortChk in $SGID
do
    if [ ${PortChk} ]; then
        IngressPortAll=$(aws ec2 describe-security-groups --filter Name=vpc-id,Values=$VPC --query "SecurityGroups[?GroupId=='${PortChk}'&&IpPermissions[?IpProtocol=='-1']].{Ingress: IpPermissions[?IpProtocol=='-1'].{IpProtocol: IpProtocol}}" --output text)
		if [ "${IngressPortAll}" ] ; then
			echo "${PortChk}" >> $RESULT_FILE
		fi
	fi
done
...
echo "Checking '0.0.0.0/0' in Security Group (Except HTTP & HTTPS)..." >> $RESULT_FILE
echo "=========================== EGRESS ===========================" >> $RESULT_FILE
for CidrChk in $SGID
do
    if [ ${CidrChk} ]; then
		EgressCidrAll=$(aws ec2 describe-security-groups --filter Name=vpc-id,Values=$VPC --query "SecurityGroups[?GroupId=='${CidrChk}'&&IpPermissionsEgress[?IpRanges[?CidrIp=='0.0.0.0/0']]&&!(IpPermissionsEgress[?IpRanges[?CidrIp=='0.0.0.0/0']&&FromPort==\`443\`])&&!(IpPermissionsEgress[?IpRanges[?CidrIp=='0.0.0.0/0']&&FromPort==\`80\`])].{Egress: IpPermissionsEgress[?IpRanges[?CidrIp=='0.0.0.0/0']].{CidrIp: IpRanges[?CidrIp=='0.0.0.0/0'].CidrIp}}" --output text)
		if [ "${EgressCidrAll}" ] ; then
			echo "${CidrChk}" >> $RESULT_FILE
		fi
	fi
done
echo "==============================================================" >> $RESULT_FILE
...
do
    if [ ${CidrChk} ]; then
        IngressCidrAll=$(aws ec2 describe-security-groups --filter Name=vpc-id,Values=$VPC --query "SecurityGroups[?GroupId=='${CidrChk}'&&IpPermissions[?IpRanges[?CidrIp=='0.0.0.0/0']]&&!(IpPermissions[?IpRanges[?CidrIp=='0.0.0.0/0']&&FromPort==\`443\`])&&!(IpPermissions[?IpRanges[?CidrIp=='0.0.0.0/0']&&FromPort==\`80\`])].{Ingress: IpPermissions[?IpRanges[?CidrIp=='0.0.0.0/0']].{CidrIp: IpRanges[?CidrIp=='0.0.0.0/0'].CidrIp}}" --output text)
		if [ "${IngressCidrAll}" ] ; then
			echo "${CidrChk}" >> $RESULT_FILE
		fi
	fi
done
...
NACLID=$(aws ec2 describe-network-acls --filter Name=vpc-id,Values=$VPC --query "NetworkAcls[].NetworkAclId" --output text)
NACL=$(aws ec2 describe-network-acls --filter Name=vpc-id,Values=$VPC --query "NetworkAcls[?Entries[?CidrBlock=='0.0.0.0/0']&&NetworkAclId=='${NACLID}'].{VpcId:VpcId, Entries:Entries[?CidrBlock=='0.0.0.0/0'&&RuleAction=='Allow'].{Cidr: CidrBlock, Action:RuleAction, Egress:Egress}}")
for NACLNAME in $NACLID
do
    if [ ${NACLNAME} ]; then
        echo "NetworkAclID: ${NACLNAME}" >> $RESULT_FILE
        if [ ${NACL} ] ; then
            echo "Result: 0.0.0.0/0 Found. 'Insecure'" >> $RESULT_FILE
        else
            echo "Result: Secure" >> $RESULT_FILE
        fi
    fi
done
...
echo "" >> $RESULT_FILE #\r\n
echo "<[3] 가상 리소스 관리> 점검 완료." >> $RESULT_FILE
